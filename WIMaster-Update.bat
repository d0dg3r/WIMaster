@echo off
echo.
echo WIMaster Update Tool
echo ===================
echo Entwickelt von Joachim Mild ^<joe@devops-geek.net^>
echo.

set Hilfe=WIMaster Update Tool
set Skript="%~d0%~p0WIMaster-Update.ps1"
...
if %version% lss 19042 (
    echo.
    echo Fehler: Dieses Tool erfordert Windows 10 20H2 oder neuer.
...
if not exist %Skript% (
    echo.
    echo Fehler: WIMaster-Update.ps1 nicht gefunden!
...
echo.
echo Fehler: Dieses Tool funktioniert nur mit Administratorrechten.
...
:Start
echo.
echo Starte WIMaster Update Tool...
echo.
...
if %errorlevel% neq 0 (
    echo.
    echo Update wurde mit Fehler beendet. Error Code: %errorlevel%
    echo.
    pause
) else (
    echo.
